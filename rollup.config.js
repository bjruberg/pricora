...
export default (CLIArgs) => {
	const bundle = {
		...
		plugins: [
			...
			html({
				title: config.frontend.pageTitle,
				fileName: 'nocache.html',
				publicPath: config.frontend.hostname + '/',
				attributes: {
					html: {
			...
			injectProcessEnv(
				...
				{ include: ['node_modules/**/*.js', '**/constants.ts'] }
			),
			...
			url({
				limit: 300,
				destDir: 'client/dist/assets',
				fileName: '[name][extname]',
				publicPath: config.frontend.hostname + '/assets/'
			})
